...
> 既知の未対応: 気象庁の府県予報区は 58 件あるが、アプリは「1都道府県=1コード」の
> 簡略化のため 47 件しか選べない。北海道の6予報区、鹿児島の奄美地方、
> 沖縄の大東島・宮古島・八重山（計11予報区）の警報には到達できない。
> 件数は `backend/tests/test_area_codes_coverage.py` で固定しており、黙って増えることはない。
