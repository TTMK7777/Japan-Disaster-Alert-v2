...
- **災害時に確認すべき交通の公式情報源を16言語の見出し付きで追加**（`/api/v1/transit-links`）。
  訪日客の災害時ニーズ調査で「日程が崩壊した」37.3%（全体4位）・「交通と空港の情報」22.2% と
  交通関連が上位に集中しているのに、アプリには交通のデータ源が1つも無かった。
  鉄道6社・主要空港4・道路・全般の計14件を収録し、**リンク先が何語で読めるかを開く前に示す**
  （タイ語話者に「日本語のみ」のページを災害のさなかに開かせないため）。
  事業者名・空港名は駅や空港の案内表示と照合できるよう訳さない。
  静的データのみで外部通信もAIも使わないため停電・低回線でも動く。
  URL は `scripts/verify_transit_links.py` で全件の HTTP ステータスを実測してから収録した
  （作成時に 404 を1件、WAF による検証不能を1件検出し、後者は収録を見送った）

- **震度の意味を16言語で表示するようにした。** 「震度4」の数字だけでは、
  日本の震度階級を知らない訪日客に何も伝わらない。一覧の各項目に
  「眠っている人も目を覚ます」「Awakens sleepers」相当の説明を出す。
  文面自体は以前から `IntensityGauge` に16言語ぶん用意されていたが、
  **import しているのがテストファイルだけで、本番のどこにも表示されていなかった**

### Fixed

- **圏外・停電時に通信が無限に走り続ける問題を修正。**
  SSE の再接続が5回失敗したあと、30秒間隔のポーリングが**上限も停止条件もなく永久に**走っていた。
  圏外では何も返ってこないので、**得る情報ゼロのまま電池だけを削る**状態だった。
  停電と圏外はこのアプリが最も要る場面であり、同時に最も電池が惜しい場面でもある。
  - `navigator.onLine === false` またはタブが非表示のときは、SSE を開かず fetch もしない
  - 復帰（`online` / `visibilitychange`）で即座に再開する
  - ポーリングは失敗が続くほど間隔を伸ばす（30秒 → 最大5分）

- **震度不明が「震度1」の配色で表示されていた問題を修正。**
  未知の震度を震度1にフォールバックしていたため、
  情報が無いだけの地震が「ほぼ揺れていない」ように見えていた。
  速報値・海外震源・P2P 由来の "不明" が該当する。無彩色の「不明」として、
  利用者の言語で明示するようにした

- **地震タブを開くたびに地図が強制表示されていた問題を修正。**
  既定はリスト表示なのに、マウント後のエフェクトが地図に上書きしていたため、
  毎回タイル取得と `fitBounds` が走っていた。地図は利用者が選んだときだけ表示する

- **E2E テスト 28 件中 12 件が壊れたまま放置されていた問題を修正。**
  初回訪問時にブラウザ言語から表示言語を推定するようにした変更（#60）以降、
  実行環境が英語だと UI が英語になり、日本語の文言でタブを探すテストが落ちていた。
  E2E は CI で実行していないため気付けなかった。Playwright のロケールを `ja-JP` に固定した。
  ダウンロード無しで実行するためのローカル用 config も追加（インストール済み Chrome を使う）

- **複数の予報区に分かれる都道府県で、大半の地域の警報が表示されていなかった問題を修正。**
  気象庁の府県予報区は 58 件あり「1都道府県=1予報区」ではない（北海道8・沖縄4・鹿児島2）が、
  各県の代表コード1つしか見ていなかった。そのため**北海道は石狩・空知・後志のみ、
  沖縄は本島のみ**で、宮古島・八重山（主要な観光地）・大東島・奄美、および北海道の7予報区の
  警報が**利用者に一切表示されていなかった**。都道府県を指定すると全予報区を並列取得して
  統合するようにした。実測（2026-08-01 北海道）では、これまで見えていなかった
  宗谷の強風注意報・根室の融雪注意報・なだれ注意報が新たに表示されるようになった。
  1予報区の取得に失敗しても他の予報区の警報は表示する
- **全国の特別警報スキャンが 11 予報区を取りこぼしていた問題を修正。**
  `get_all_prefectures_warnings` が都道府県表（47件）を回っていたため、上記11予報区で
  特別警報が出ても検出されず、**Push 通知も飛ばない**状態だった。府県予報区58件すべてを回る

- **継続中の気象警報がすべて表示されていなかった問題を修正（最重要）。** 気象庁は警報を
  最初に出したときだけ `status` を「発表」にし、その後の定時更新では「継続」に変える。
  実装は「発表」だけを通していたため、**発表直後の一瞬しか警報が表示されず、
  継続中の警報は消えていた**。実測（2026-08-01 沖縄）では継続62件・解除29件・発表0件で、
  有効な注意報がありながら表示はゼロだった
- **鹿児島県の警報が常にゼロ件だった問題を修正。** 設定していた地域コード `460000` は
  気象庁に存在せず、404 → 例外 → 空リストになっていた。エラー表示も無いため気付けない状態だった。
  気象庁は鹿児島を「奄美地方」と「それ以外」の2予報区に分けており、本土側の `460100` に変更した
- **10言語（th / id / ms / tl / fr / de / it / es / ne / zh-TW）が16言語対応の恩恵を受けていなかった
  問題を修正。** `STATIC_LANGUAGES` が6言語しか無く、残る10言語は AI 経路に流れていたため、
  16言語化した警報名・行動指示が本番では使われず、AI 不在時は英語固定になっていた。
  全16言語を静的経路に統一し、AI・ネットワークに依存せず同じ結果が出るようにした
- **気象警報の地域名が全言語で日本語のままだった問題を修正。** 気象庁の警報 JSON には
  地域名が入っておらず `code` しか無いのに、実装は `area.get("name")` を読んでいたため
  常に都道府県名（日本語）にフォールバックしていた。日本語名をキーにした旧 `AREA_TRANSLATIONS`
  （4地域×5言語）は一度も引かれない死蔵コードだった。気象庁公式の地域マスタから
  **193地域の日本語名・英語名を生成**し、コードから解決するようにした。
  あわせて表示の粒度が都道府県から一次細分区域に上がった（例「東京都」→「伊豆諸島北部」）
